...
var make_tree = function (numbers) {
    ...
    numbers.forEach(function (nb) {
        nb.toString().split('').reduce(function (parentNode, digit, index) {
...
var make_regex = function (numbers_tree) {
    ...
    var makeLogic = function (subtree) {
        if (subtree === {})
            ...
            return keys.map(function (n) { return n.toString(); }).map(function (key) {
    ...
    return notGroupedRegex.replace(/\((\d\|?)+\)/g, function (match) {
        var groups = [];
        match.substring(1, match.length - 1).split('|').reduce(function (groupIndex, n) {
            var nbr = parseInt(n);
            if (groupIndex === -1) {
                groups.push([nbr]);
                return groupIndex + 1;
        ...
        return "(".concat(groups.map(function (list) { return list.length === 1 ? list[0].toString() : "[".concat(list[0], "-").concat(list[list.length - 1], "]"); }).join('|'), ")");
    });
};
/**
 * Creates a regex from an array of numbers
 */
var regexFromNumberArray = function (array) { return make_regex(make_tree(array)); };
 ...
 */
var simplifyExistingRegex = function (regex) { return regex.replace(/^((\d+|(\(\d+\)))\|?)+$/g, function (match) {
    return (0, exports.regexFromNumberArray)(match.split('|').map(function (s) { return s.replace(/(^\()|(\)$)/g, ''); }).map(function (s) { return parseInt(s); }));
}).replace(/\(((\d+|(\(\d+\)))\|?)+\)/g, function (match) {
    match = match.substring(1, match.length - 1);
    var simplified = (0, exports.regexFromNumberArray)(match.split('|').map(function (s) { return s.replace(/(^\()|(\)$)/g, ''); }).map(function (s) { return parseInt(s); }));
