const make_tree = (numbers : number[]) => {
    const tree: any= {};
    numbers.forEach(nb => {
        nb.toString().split('').reduce((parentNode, digit, index) => {
...
const make_regex = (numbers_tree: any) => {
    ...
    const makeLogic = (subtree: any) => {
        if(subtree === {}) return ''
            ...
            keys.sort((a,b) => a - b)
            return keys.map(n => n.toString()).map(key =>{
    ...
    return notGroupedRegex.replace(/\((\d\|?)+\)/g, match => {
        const groups: number[][] = []
        match.substring(1,match.length-1).split('|').reduce((groupIndex, n) => {
        ...
        return `(${groups.map(list => list.length === 1 ? list[0].toString() : `[${list[0]}-${list[list.length-1]}]`).join('|')})`
    })
}
 ...
 */
export const regexFromNumberArray = (array: number[]) => make_regex(make_tree(array))
 ...
 */
export const simplifyExistingRegex = (regex: string) => regex.replace(/^((\d+|(\(\d+\)))\|?)+$/g, (match) => {
    return regexFromNumberArray(match.split('|').map(s => s.replace(/(^\()|(\)$)/g, '')).map(s => parseInt(s)))
}).replace(/\(((\d+|(\(\d+\)))\|?)+\)/g, (match) => {
